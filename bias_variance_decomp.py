    ...
    **fit_params
    ...
    for i in range(num_rounds):
        ...
        if estimator.__class__.__name__ in ["Sequential", "Functional"]:
            ...
            pred = estimator.fit(X_boot, y_boot, **fit_params).predict(X_test).reshape(1, -1)
        all_pred[i] = pred
...
def bias_variance_decomp_pno(
    ...
    **fit_params
    ...
    if loss == "0-1_loss":
        ...
        dtype = np.float64
    all_pred_set = [np.zeros((num_rounds, y_test[0].shape[0]), dtype=dtype) for i in range(len(y_test))]
    # all_pred = np.zeros((num_rounds, y_test.shape[0]), dtype=dtype)
    for i in range(num_rounds):
        X_boot, y_boot = _draw_bootstrap_sample(rng, X_train, y_train)
        ...
        if estimator.__class__.__name__ in ["Sequential", "Functional"]:
            ...
            for X in X_test:
                ...
                all_pred_set.append(pred)
        else:
            estimator.fit(X_boot, y_boot, **fit_params)
            for i_set ,X in enumerate(X_test):
                pred = estimator.predict(X).reshape(1, -1)
                all_pred_set[i_set][i] = pred


    avg_var_sets = []
    avg_bias_sets = []
    avg_expected_loss_sets = []
    ...
    if loss == "0-1_loss":
        ...
        for all_pred,y in zip(all_pred_set,y_test):
            avg_expected_loss = np.apply_along_axis(
                lambda x: ((x - y) ** 2).mean(), axis=1, arr=all_pred
            ).mean()

            main_predictions = np.mean(all_pred, axis=0)

            avg_bias = np.sum((main_predictions - y_test) ** 2) / y.size
            avg_var = np.sum((main_predictions - all_pred) ** 2) / all_pred.size
            avg_var_sets.append(avg_var)
            avg_bias_sets.append(avg_bias)
            avg_expected_loss_sets.append(avg_expected_loss)

    return avg_var_sets, avg_bias_sets, avg_expected_loss_sets