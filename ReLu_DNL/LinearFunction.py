...
class LinearFunction:
    ...
    def is_same(self, lin_func_2):
        ...
        :return:
        """
        return is_same_slope(self.slope, lin_func_2.slope)
...
def is_same_slope(slope_1, slope_2):
    """
    given parameters of two linear function, evaluates whether they are same.
    Right not there are some division errors and exact comparison fails to prove to be robust.
    Not checking constants for now, it is not likely to have same line. Otherwise floating number arithmetics cause problems.
    ...
    # print("sum: ",np.abs(np.minimum(slope_1,slope_2)))
    # print("log: ",np.log(np.abs(np.minimum(slope_1,slope_2))))
    #
    ...
    #     return False
    precision_factor = 0
    precision = np.maximum(int(np.log(np.abs(np.minimum(slope_1, slope_2)))) + precision_factor, 0)
    if np.around(slope_1, precision) == np.around(slope_2, precision):
        return True
    else:
        return False
...
def get_lin_func_param(point_1, point_2):
    ...
    slope = (point_1.predicted_profit - point_2.predicted_profit) / (point_1.x - point_2.x)
    constant = point_1.predicted_profit - point_1.x * slope
    return slope, constant
