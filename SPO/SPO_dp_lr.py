...
class SGD_SPO_dp_lr:
    def __init__(self, capacity=None, weights=None, epochs=2, layer_params = None, doScale= True, early_stopping= False,
        ...
        degree=1, optimizer= optim.SGD,store_result =False, path=None,**hyperparam):
        ...
        self.time = 0
        self.best_model_state = 0
        self.best_val_regret =float("inf")
                ...
                if self.verbose or self.plotting or self.store_result:
                    ...
                    if cnt % 20 == 0:
                        ...
                        if validation:
                            ...
                            tmp_val = dict_validation['regret_full']
                            if tmp_val < self.best_val_regret:
                                self.best_model = copy.deepcopy(self)
                                self.best_val_regret = tmp_val
                        if test:
    ...
    def predict(self, x_test, qid=True):
        if qid:
    ...
    def save_spo_model(self, f_name=None):
...
class Pytorch_regression:
    def __init__(self, capacity=None, weights=None, epochs=10,early_stopping= False, validation_relax= False,use_dp=True,
        doScale=True, n_items=48,model=None, verbose=False,path=None, plotting=False,return_regret= False,store_result =False,
    ...
    def fit(self, x_train, y_train,x_validation=None,y_validation=None,x_test=None,y_test=None):
        ...
        if self.return_regret:
            ...
            if validation:
                ...
                tmp_val = dict_validation['regret']
                # if tmp_val < self.
